  ...
  if (allowed('invoices.read')) {
    ...
    if (overdue._count._all > 0) {
      alerts.push({
        ...
        detail: 'Leur echeance est depassee et elles ne sont pas soldees.',
  ...
  if (allowed('quotes.read')) {
    ...
    if (expiring > 0) {
      alerts.push({
        ...
        kind: 'QUOTE_EXPIRING',
        severity: 'warning',
        title: `${expiring} devis expire(nt) bientot`,
        detail: `Leur validite s'acheve dans moins de ${QUOTE_WARNING_DAYS} jours.`,
  ...
  if (allowed('purchases.read')) {
    ...
    if (due._count._all > 0) {
      alerts.push({
        ...
        kind: 'PAYABLE_DUE',
        severity: 'warning',
        title: `${due._count._all} commande(s) fournisseur a regler`,
        detail: 'Leur echeance de paiement est depassee.',
  ...
  if (allowed('cash.read')) {
    ...
    if (stale > 0) {
      alerts.push({
        ...
        detail: 'Fermez la caisse pour rapprocher les especes comptees.',
        href: '/caisse',
